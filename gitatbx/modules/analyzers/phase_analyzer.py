...
Outputs per-condition plot-ready parquets with:
  ...
  y_data: mean PSD across epochs (dB re 1 µV²/Hz)
  y_var:  SEM across epochs (dB)
...
def compute_phase(ip: str,
    ...
    """Compute mean power spectral density (dB) per condition from epoched EEG.
    ...
    def _psd_for_channels(eids: list, ch_list: list) -> tuple[list, list] | None:
        """Mean PSD (dB) + SEM across epochs for a set of channels."""
        epoch_psds = []
        for eid in eids:
        ...
        mean_db = (10 * np.log10(np.mean(arr, axis=0) + eps)).tolist()
        sem_db  = (10 * np.log10(np.std(arr, axis=0, ddof=1) / np.sqrt(len(arr)) + eps)).tolist() if len(arr) > 1 else [0.0] * len(freq_vals)
...
if __name__ == '__main__':
    (lambda a: compute_phase(
        ...
        print('[phase] Mean power spectrum (dB) per condition. Plot-ready output.'),
