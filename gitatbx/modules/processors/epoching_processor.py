...
def _epoch_mne(raw, events: Dict[str, List[Tuple[float, float]]], data_path: str, rec_start: float = 0.0, event_sfreq: float | None = None) -> str:
    ...
    (pl.concat(dfs) if dfs else pl.DataFrame()).write_parquet(out, compression='gzip')
...
def window_epochs(data_path: str, window_size: float = 30.0, step_size: float = 10.0, crop_start: float = 0.0, crop_end: float = 0.0) -> str:
    ...
    if missing:
        ...
        pl.DataFrame({'signal': [1]}).write_parquet(signal_path, compression='gzip')
        return signal_path
    ...
    if not windowed_dfs:
        ...
        pl.DataFrame(empty).write_parquet(out, compression='gzip')
    ...
    base = os.path.splitext(os.path.basename(data_path))[0]
    out = f"{base}_windowed.parquet"
    result_df.write_parquet(out, compression='gzip')
