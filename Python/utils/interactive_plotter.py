...
def add_log_to_archive(archive_path, participant_id, log_path, log_name):
    ...
    if participant_id == 'global':
        ...
        sidecar_dir = archive_dir                             # inside .bin/
        relative_file = '.bin/' + log_filename               # /.bin/EV.log.parquet after loadSidecar prepends /
    else:
        log_filename = f'{participant_id}.log.parquet'        # e.g. EV_002.log.parquet
        sidecar_dir = os.path.join(archive_root, participant_id)  # results root / EV_002/
        relative_file = f'{participant_id}/{log_filename}'    # /EV_002/EV_002.log.parquet
...
def run(inp, out_dir, pre, project_name='procedure'):
    ...
    # Copy parquet to sidecar location: <out_dir>/<pid>/plots/<pre>.parquet
    # Sidecars stay at the results root (out_dir), not inside .bin/ where the HTML lives
    participant_plots_dir = os.path.join(out_dir_abs, participant_id, 'plots')
