...
def _write_launcher_sidecar(archive_path):
    ...
    serve_py = f'''\
#!/usr/bin/env python3
# Auto-generated by EmotiView pipeline.
# Run: python3 .bin/{serve_name}   (or double-click {base}_results.sh)
import os, json, threading, webbrowser, http.server, socketserver
...
'''
    ...
    sh = (
        ...
        'echo\n'
        f'python3 ".bin/{serve_name}" || python ".bin/{serve_name}"\n'
    )

    sh_path = os.path.join(archive_dir, f'{base}_results.sh')
    ...
    try:
        ...
        try:
            ...
            pass
        # Remove any stale root-level serve.py or bat from old generator versions
        for stale_name in [serve_name, base + '_results.bat']:
            stale = os.path.join(archive_dir, stale_name)
            if os.path.isfile(stale):
                try:
                    os.remove(stale)
                except Exception:
                    pass
        stale = os.path.join(archive_dir, serve_name)
        if os.path.isfile(stale):
            try:
                os.remove(stale)
            except Exception:
                ...
                if (metaResp?.ok) {{
                    ...
                    if (loaded && typeof loaded === 'object') {{
                        ...
                        if (!meta['global_log']) {{
                            ...
                            if (gHead?.ok)
                                meta['global_log'] = {{ title: 'Pipeline Log', path: ['global', 'log'], type: 'log', file: '{project_name}.log.parquet' }};
                        }}
                        return meta;
                    }}
                }}
...
def update_archive(archive_path, participant_id, plot_id, plot_title, tree_path):
    ...
    try:
        ...
        existing_meta[plot_id] = {
            ...
            'path': tree_path,
            'type': 'plot',
            'file': f'{participant_id}/plots/{plot_id}.parquet'
        }
